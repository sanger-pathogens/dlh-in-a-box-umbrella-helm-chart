...
- optionally renders Mermaid blocks with Docker via `validate_mermaid.py`
...
- `SKIP_MERMAID_CHECK=1` skips the renderer step from `docs-check.sh`
...
- treating `smoke-install.sh` as equivalent to a simple manual `helm install`
