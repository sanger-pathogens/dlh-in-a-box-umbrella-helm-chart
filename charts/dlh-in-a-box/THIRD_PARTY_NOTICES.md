...
| `datahub-prerequisites` Helm chart and bundled dependency charts | `0.2.3` and the upstream prerequisite charts it redistributes | Apache-2.0 | Packaged upstream chart archive so the umbrella release can deploy DataHub and its prerequisites together. |
