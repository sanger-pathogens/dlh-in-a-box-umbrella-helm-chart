...
def export_diagrams(
    *,
    ...
    for key, diagram_spec in source.diagrams.items():
        ...
        download_file(export["fileUrls"]["png"], output_path, expected_header=b"\x89PNG\r\n\x1a\n")
...
def main() -> int:
    ...
    parser.add_argument("--pdf-output-dir", default=DEFAULT_PDF_OUTPUT_DIR)
    parser.add_argument("--pdf-filename", default=DEFAULT_PDF_FILENAME)
    ...
    if not pdf_output_dir.is_absolute():
        pdf_output_dir = repo_root / pdf_output_dir
    ...
    print(f"{'Checked' if args.dry_run else 'Exported'} {len(exported)} assets")
    return 0
