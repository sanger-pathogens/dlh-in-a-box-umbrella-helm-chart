...
| `DLH-C2-PUBLISH` | Chart Publish Automation | `App` | GitHub Actions / Helm | `.github/workflows/helm-publish.yaml`; `scripts/package.sh` | Release automation that builds versioned Helm artifacts and publishes them for deployment repositories. |
...
| --- | --- | --- | --- | --- |
| `DLH-C3-VALIDATE-LINT` | Helm Lint Workflow | `DLH-C2-VALIDATION` | `.github/workflows/helm-lint.yaml`; `scripts/lint.sh` | Runs automated checks for licensing, documentation, security, rendering rules, shell scripts, JSON schema, and Helm syntax. |
| `DLH-C3-VALIDATE-DEPS` | Dependency Refresh Check | `DLH-C2-VALIDATION` | `scripts/helm-dependency-update.sh`; `Chart.lock` | Refreshes Helm dependency archives and updates lock metadata so packaged dependencies remain reproducible. |
| `DLH-C3-VALIDATE-RENDER` | Example Render Checks | `DLH-C2-VALIDATION` | `scripts/template.sh`; `examples/*.yaml` | Renders every maintained example configuration file to verify that supported installation profiles produce valid manifests. |
...
| `DLH-C3-VALIDATE-SMOKE` | Smoke Install Workflow | `DLH-C2-VALIDATION` | `.github/workflows/helm-smoke-install.yaml`; `scripts/smoke-install.sh` | Installs an identity- and governance-oriented local profile into a disposable cluster to test an integrated runtime path. |

### Publish Components

| ID | Name | Parent | Path | Description |
| --- | --- | --- | --- | --- |
| `DLH-C3-PUBLISH-PACKAGE` | Helm Package Step | `DLH-C2-PUBLISH` | `scripts/package.sh`; `dist/` | Builds the versioned Helm chart archive for release. |
